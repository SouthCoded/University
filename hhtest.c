...
void (*allocators[])(size_t) = {&f00, &f01, &f02, &f03, &f04, &f05, &f06, &f07,
...
static void phase(double skew, unsigned long long count) {
  ...
  for (unsigned long long i = 0; i < count; ++i) {
    ...
    prev_state = setstate(prev_state);
    allocators[r](sizes[r]);
    prev_state = setstate(prev_state);
  }
}

static inline size_t get_bytes(unsigned long long idx,
                               unsigned long long count) {
  return random() % (count / 2) == 0 ? idx * sizes[NALLOCATORS - 8] : 1;
}

static inline int get_line_number(unsigned long long idx,
                                  unsigned long long count) {
  return idx + 1 + (random() % (4 * count) + random() % idx);
}

static void many_file_line_pairs(unsigned long long count) {

  const int ALLOCATIONS_PER_PAIR = 100;
  char state_buff[STATE_BUFF_LEN];
  char *prev_state = initstate(0, state_buff, STATE_BUFF_LEN);
  ...
  for (unsigned long long i = 1; i < count + 1; i++) {
    line_number = get_line_number(i, count);
    bytes = get_bytes(i, count);
    prev_state = setstate(prev_state);

    for (int j = 0; j < ALLOCATIONS_PER_PAIR; j++) {
      void *ptr = cs0019_malloc(bytes, __FILE__, line_number);
      free(ptr);
    }
...
\n\
  The -l option will test whether your implementation can handle PAIRCOUNT \n\
  (file, line) pairs.\n");
    exit(0);
  } else if (argc > 1 &&
             (strcmp(argv[1], "-l") == 0 || strcmp(argv[1], "--load") == 0)) {
    unsigned long long pairs = 10000;
    if (argc == 3) {
      pairs = strtoull(argv[2], 0, 0);
    }
    many_file_line_pairs(pairs);
